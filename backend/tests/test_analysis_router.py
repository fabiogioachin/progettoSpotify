...
class TestAnalysisRouter:
    ...
    def test_start_analysis_requires_auth(self):
        """POST /api/v1/analyze-tracks senza auth deve restituire 401."""
        response = client.post(
            "/api/v1/analyze-tracks",
            json={"track_ids": ["abc123"]},
        )
        assert response.status_code == 401

    def test_get_status_requires_auth(self):
        """GET /api/v1/analyze-tracks/{id} senza auth deve restituire 401."""
        response = client.get("/api/v1/analyze-tracks/00000000-0000-0000-0000-000000000000")
        assert response.status_code == 401
